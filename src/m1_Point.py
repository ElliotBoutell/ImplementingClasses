...
class Point(object):
    ...
    def move_to(self, x, y):
        original_point = self.clone()
        self.x = x
        self.y = y
        self.moves = self.moves + 1
        distance = self.get_distance_from(original_point)
    ...
    def get_distance_from_start(self):
        start = Point(self.static_x, self.static_y)
        distance = self.get_distance_from(start)
        return distance
    ...
    def closer_to(self, p2, p3):
        leg1 = self.x - p2.x
        leg2 = self.y - p3.y
        distance1 = sqrt((leg1 ** 2) + (leg2 ** 2))
        leg1 = self.x - p2.x
        leg2 = self.y - p2.y
        distance2 = sqrt((leg1 ** 2) + (leg2 ** 2))
